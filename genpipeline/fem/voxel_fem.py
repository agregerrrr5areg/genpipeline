...
class VoxelHexMesher:
    ...
    def run_ccx(inp_path: str, ccx_cmd: str = None, timeout: int = 300) -> Dict[str, float]:
        ...
        with _CCX_SEMAPHORE:
            logger.info(f"[VoxelFEM] Running (Active Slots: {multiprocessing.cpu_count()//2}): {' '.join(cmd)}")
            try:
                proc = subprocess.run(
...
class VoxelFEMEvaluator:
    ...
    def evaluate_batch(self, param_list: list) -> list:
        ...
        def eval_one(idx, p):
            ...
            if z is None:
                return idx, FEMResult(stress_max=FEM_SENTINEL, compliance=FEM_SENTINEL, mass=1.0, success=False), None
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=n_workers) as executor:
            future_to_idx = {executor.submit(eval_one, i, p): i for i, p in enumerate(param_list)}
