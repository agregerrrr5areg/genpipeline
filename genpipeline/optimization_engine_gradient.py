...
class LatentGradientOptimizer:
    ...
    def optimize(self, z_init: np.ndarray, sim_cfg: dict, 
                 n_steps: int = 20, 
                 markov_weight: float = 0.1,
                 volfrac: float = 0.4):
        # Forward to batch implementation with size 1
        return self.optimize_batch(z_init[None, :], sim_cfg, n_steps, markov_weight).squeeze()

    def optimize_batch(self, z_batch_init: np.ndarray, sim_cfg: dict, 
                       n_steps: int = 20, 
                       markov_weight: float = 0.1):
        """
        ...
        z = torch.from_numpy(z_batch_init).float().to(self.device).clone().requires_grad_(True)
        optimizer = torch.optim.Adam([z], lr=self.lr)
        
        # Setup static solvers
        res = 32
        nx, ny, nz = res, res//4, res//4
        if sim_cfg.get("geometry_type") == "lbracket": nz = res
        
        solver = SIMPSolverGPU(nx=nx, ny=ny, nz=nz, boundary_conditions=sim_cfg.get("boundary_conditions"))
        ...
        for step in range(n_steps):
            ...
            voxels_32 = F.interpolate(logits, size=(nx, ny, nz), mode='trilinear', align_corners=False)
            ...
            loss_markov = (torch.abs(xPhys_batch[:, :, 1:, :, :] - xPhys_batch[:, :, :-1, :, :]).mean() +
                           torch.abs(xPhys_batch[:, :, :, 1:, :] - xPhys_batch[:, :, :, :-1, :]).mean() +
                           torch.abs(xPhys_batch[:, :, :, :, 1:] - xPhys_batch[:, :, :, :, :-1]).mean())
            ...
            if (step + 1) % 10 == 0:
                logger.info(f"  Step {step+1:2d} | Batch Gradient Norm: {z.grad.norm().item():.4f}")
