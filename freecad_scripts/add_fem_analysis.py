#!/usr/bin/env python3
"""
FreeCAD script to add FEM analysis to existing designs for topology optimization.
This script will:
1. Open an existing FreeCAD file
2. Create FEM analysis setup
3. Add material properties
4. Apply loads and constraints
5. Run the analysis
6. Save the results back to the file
"""
...
def add_fem_analysis(doc_path, analysis_name="TopologyOptimization"):
    """Add FEM analysis to a FreeCAD document"""
    # Open the document
    ...
    try:
        ...
        doc.saveAs(doc_path.replace(".FCStd", "_with_fem.FCStd"))
        print(
            f"Document saved with FEM analysis: {doc_path.replace('.FCStd', '_with_fem.FCStd')}"
        )
...
def main():
    ...
    for fcstd_file in fcstd_files:
        ...
        success = add_fem_analysis(fcstd_file)
        if success:
            print(f"Successfully added FEM analysis to: {fcstd_file}")
        else:
            print(f"Failed to add FEM analysis to: {fcstd_file}")
